...
pipeline {
  ...
  environment {
      IMAGE_NAME = 'caleidos/karina-web'
    ...
    stage ('Deploy') {
      steps {
        script {
          def imageTag = "${IMAGE_TAG}"
          def imageName = "${IMAGE_NAME}:${imageTag}"
          def containerName = "karina"
          def containerPort = 80
          def applicationName = "AppECS-KarinaWeb-karina"
          def deploymentGroupName = "DgpECS-KarinaWeb-karina"
          def taskDefinitionName = "karina-web"

          sh  "                                                                     \
          sed -e 's;%REPO%;${imageName};g'\
              -e 's;%AWSACCOUNT%;${AWS_ACCOUNT};g'\
