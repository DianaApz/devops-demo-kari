...
pipeline {
  ...
  stages {
    ...
    stage("Deploy") {
      steps {
        script {
          DEPLOYMENT_ID = sh (returnStdout: true, script: 'aws deploy create-deployment --application-name Karina --deployment-group-name DEV --s3-location bucket=ci-workshop-devops,key=karina/Artifact/$BUILD_TAG.zip,bundleType=zip --region us-east-1').trim()     
